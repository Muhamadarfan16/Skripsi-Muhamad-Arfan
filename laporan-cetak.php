...
while ($row1x = $stmt1x->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(40,7,$row1x['nama_pegawai'],1,0,'L');
	$stmtrx = $pro->readR($row1x['id_pegawai']);
...
while ($row1y = $stmt1y->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(40,7,$row1y['nama_pegawai'],1,0,'L');
	$stmtry = $pro->readR($row1y['id_pegawai']);
...
while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)){
	$pdf->Cell(25,7,$row1['nama_pegawai'],1,0,'L');
	$stmtr = $pro->readR($row1['id_pegawai']);
	while ($rowr = $stmtr->fetch(PDO::FETCH_ASSOC)){
		$pdf->Cell(25,7,number_format($rowr['bobot_normalisasi'], 3, '.', ','),1,0,'L');
	}
	$pdf->Cell(40,7,$row1['hasil_pegawai'],1,0,'L');
	$pdf->ln();
}
